...
public class AlipaySubmit {
    ...
    @Autowired
    private net.coupirum.io.httprequest.HttpRequest httpRequest;
    ...
    /**
     * 建立即时到账批量退款有密接口(refund_fastpay_by_platform_pwd)请求, 以 http request POST方式提交表单
     *
     * @param batchNo 退款批次号, 必填(时间格式是yyyyMMddHHmmss+数字或者字母)
     * @param batchNum 退款笔数, 必填(值为您退款的笔数,取值1~1000间的整数)
     * @param detailData 退款详细数据, 必填(支付宝交易号^退款金额^备注)多笔请用#隔开
     * @return 提交表单HTML文本
     */
    public String httpRequestRefund(String batchNo, String batchNum, String detailData) {
        String result;
