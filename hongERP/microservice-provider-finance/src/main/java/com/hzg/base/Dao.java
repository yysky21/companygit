...
//        boundValueOperations.rename("BoundTest123");
     ...
     * @return
     */
    public String delPart(Class clazz, Integer id){
        String delSql = "delete from " + ((Entity)clazz.getAnnotation(Entity.class)).name() + " where id < "+ id;
        int result =  sessionFactory.getCurrentSession().createSQLQuery(delSql).executeUpdate();
        return (result > 0 ? CommonConstant.success : CommonConstant.fail);
    }
     ...
     * @return
     */
    public List queryPart(Class clazz, Integer id){
        String querySql = "select * from " + ((Entity)clazz.getAnnotation(Entity.class)).name() + " where id < "+ id;
        List results =  sessionFactory.getCurrentSession().createSQLQuery(querySql).list();
        return results;
    }
     ...
     * @return
     */
    public String delAll(Class clazz){
        String delSql = "delete from " + ((Entity)clazz.getAnnotation(Entity.class)).name();
        int result =  sessionFactory.getCurrentSession().createSQLQuery(delSql).executeUpdate();
        return (result > 0 ? CommonConstant.success : CommonConstant.fail);
    }
    ...
    public List queryAll(Class clazz){
        String querySql = "select * from " + ((Entity)clazz.getAnnotation(Entity.class)).name();
        List results =  sessionFactory.getCurrentSession().createSQLQuery(querySql).list();
        return results;
    }
     ...
     * @return
     */
    public List queryAllYesterday(Class clazz,String fieldName){
        String querySql = "select * from " + ((Entity)clazz.getAnnotation(Entity.class)).name() + "where date(" + fieldName + ") = date_sub(curdate(),interval 1 day)";
        List results =  sessionFactory.getCurrentSession().createSQLQuery(querySql).list();
        return results;
    }
}
