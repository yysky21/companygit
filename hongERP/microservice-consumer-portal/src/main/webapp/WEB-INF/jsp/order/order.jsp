                            ...
                            <div class="col-md-12 col-md-offset-1" id="submitDiv">
                                ...
                                <c:if test="${entity.state == 4}">
                                    <c:if test="${fn:contains(resources, '/afterSaleService/business/returnProduct')}">
                                        <button id="returnProduct" type="button" class="btn btn-success">申请退货</button>
                                    </c:if>
                                </c:if>
                                <c:if test="${entity.state == 4}">
                                    <c:if test="${fn:contains(resources, '/afterSaleService/business/changeProduct')}">
                                        <button id="changeProduct" type="button" class="btn btn-success">申请换货</button>
                                    </c:if>
                                </c:if>
                                <c:if test="${entity.state == 4}">
                                    <c:if test="${fn:contains(resources, '/afterSaleService/business/repairProduct')}">
                                        <button id="repairProduct" type="button" class="btn btn-success">申请修补货品</button>
                                    </c:if>
                                </c:if>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    <c:if test="${entity.state == 4 && fn:contains(resources, '/afterSaleService/business/repairProduct')}">
        $("#repairProduct").click(function(){render("<%=request.getContextPath()%>/afterSaleService/business/repairProduct?json=" + encodeURI('{"entityId":${entity.id}, "entity":"order"}'));});
    </c:if>
</script>