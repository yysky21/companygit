...
public class ErpController extends com.hzg.base.Controller {
    ...
    public String viewById(Map<String, Object> model, @PathVariable("entity") String entity, @PathVariable("id") Integer id,
        ...
        if (entity.equalsIgnoreCase(Purchase.class.getSimpleName())) {
            ...
            entities = writer.gson.fromJson(client.query(entity, json), new TypeToken<List<BorrowProduct>>() {}.getType());
            if (entities.isEmpty()) {
                Map<String, String> no = writer.gson.fromJson(erpClient.getNo(ErpConstant.no_borrowProduct_perfix), new TypeToken<Map<String, String>>() {
                }.getType());
                model.put(CommonConstant.no, no.get(CommonConstant.no));
            }

        } else if (entity.equalsIgnoreCase(BorrowProductReturn.class.getSimpleName())) {
            entities = writer.gson.fromJson(client.query(entity, json), new TypeToken<List<BorrowProductReturn>>() {}.getType());
            if (entities.isEmpty()) {
                Map<String, String> no = writer.gson.fromJson(erpClient.getNo(ErpConstant.no_borrowProductReturn_perfix), new TypeToken<Map<String, String>>() {
                }.getType());
                model.put(CommonConstant.no, no.get(CommonConstant.no));
            }
        }
