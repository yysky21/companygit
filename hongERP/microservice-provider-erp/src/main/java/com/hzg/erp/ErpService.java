...
public class ErpService {
    ...
    public float getProductUnsaledQuantity(Product product) {
        return new BigDecimal(Float.toString(getProductQuantity(product))).
                subtract(new BigDecimal(Float.toString(getProductSoldQuantity(product)))).floatValue();
    }
            ...
            for (StockInOutDetail detail : stockOut.getDetails()) {
                for (StockInOutDetailProduct detailProduct : detail.getStockInOutDetailProducts()) {
                    ...
                    if (dbProduct.getState().compareTo(ErpConstant.product_state_stockIn) != 0 &&
                            dbProduct.getState().compareTo(ErpConstant.product_state_stockIn_part) != 0 &&
                            dbProduct.getState().compareTo(ErpConstant.product_state_mediaFiles_uploaded) != 0 &&
                            dbProduct.getState().compareTo(ErpConstant.product_state_edit) != 0 &&
                            dbProduct.getState().compareTo(ErpConstant.product_state_stockOut_part) != 0 &&
...
//            orderReqDto.setPayArea(sfExpress.getPayArea());
    ...
    public ExpressDeliverDetail queryLastExpressDeliverDetailByProduct(Product product) {
        ...
        List<ExpressDeliverDetailProduct> detailProducts = erpDao.query(detailProduct);
        ExpressDeliverDetail deliverDetail = detailProducts.isEmpty() ? null : detailProducts.get(0).getExpressDeliverDetail();
        if (deliverDetail != null) {
            deliverDetail.setExpressDeliver((ExpressDeliver) erpDao.queryById(deliverDetail.getExpressDeliver().getId(), deliverDetail.getExpressDeliver().getClass()));
        }
    ...
    public Float getExpressDeliverDetailWeight(ExpressDeliverDetail detail) {
        ...
        if (detail.getUnit().equals(ErpConstant.unit_g)) {
            ...
            /**
             * 商品重量单位为 克, 商品重量属性在采购录入时，有可能会带上g，如300.5克，录作：300.5g，这里获取数字
             */
            if (productWeight != null) {
                detailWeight = new BigDecimal(productWeight.replaceAll("[^-0-9?!.]","")).multiply(new BigDecimal("0.001")).doubleValue();
            }
        }
    ...
    public String saveBorrowProduct(BorrowProduct borrowProduct) {
        String result = CommonConstant.fail;

        String msg = isCanSaveBorrowProduct(borrowProduct);
        if (msg != null) {
            return CommonConstant.fail + msg;
        }

        borrowProduct.setState(ErpConstant.borrowProduct_state_save);
        borrowProduct.setInputDate(dateUtil.getSecondCurrentTimestamp());
        borrowProduct.setBorrower(getUserBySessionId(borrowProduct.getSessionId()));
        result += erpDao.save(borrowProduct);
        result += saveBorrowProductDetails(borrowProduct);

        return result.equals(CommonConstant.fail) ? result : result.substring(CommonConstant.fail.length());
    }


    public String isCanSaveBorrowProduct(BorrowProduct borrowProduct) {
        String msg = null;

        for (BorrowProductDetail detail : borrowProduct.getDetails()) {
            Product queryProduct = new Product(detail.getProductNo());

            float unsaledProductQuantity = getProductUnsaledQuantity(queryProduct);
            if (unsaledProductQuantity < detail.getQuantity()) {
                return "商品:" + detail.getProductNo() + "借货数量" + detail.getQuantity() + "大于实际已有数量:" + unsaledProductQuantity + ",保存失败";
            }
            ...
            if (detail.getUnit().equals(ErpConstant.unit_g) || detail.getUnit().equals(ErpConstant.unit_kg) ||
                ...
                productCount = detail.getQuantity().intValue();
            }

            List<Product> products = erpDao.query(queryProduct);
            if (productCount > products.size()) {
                return "商品:" + detail.getProductNo() + "保存件数" + detail.getQuantity() + "大于实际件数:" + products.size() + ",保存失败";
            }
        }
    ...
    public String updateBorrowProduct(BorrowProduct borrowProduct) {
        String result = CommonConstant.fail;
        ...
        if (dbBorrowProduct.getState().compareTo(ErpConstant.borrowProduct_state_save) == 0) {
            ...
            for (BorrowProductDetail detail : dbBorrowProduct.getDetails()) {
                BorrowProductDetailProduct detailProduct = new BorrowProductDetailProduct();
                detailProduct.setBorrowProductDetail(detail);
                List<BorrowProductDetailProduct> detailProducts = erpDao.query(detailProduct);

                for (BorrowProductDetailProduct dbDetailProduct : detailProducts) {
            ...
            result += CommonConstant.fail + ", 借货单 " + dbBorrowProduct.getNo() + " 不是保存状态，不能修改";
        }
    ...
    public String saveBorrowProductDetails(BorrowProduct borrowProduct) {
        String result = CommonConstant.fail;
        ...
        for (BorrowProductDetail detail : borrowProduct.getDetails()) {
            ...
            List<Product> products = erpDao.query(new Product(detail.getProductNo()));
            BorrowProductDetail idDetail = new BorrowProductDetail(detail.getId());
            for (int i =0; i < detailCount; i++) {
                BorrowProductDetailProduct detailProduct = new BorrowProductDetailProduct();
                detailProduct.setProduct(products.get(i));
                detailProduct.setBorrowProductDetail(idDetail);
                result += erpDao.save(detailProduct);
            }
        }
    ...
    public List<BorrowProduct> queryBorrowProduct(BorrowProduct borrowProduct) {
        List<BorrowProduct> borrowProducts = erpDao.query(borrowProduct);

        for (BorrowProduct ele : borrowProducts) {
            for (BorrowProductDetail detail : ele.getDetails()) {
                BorrowProductDetailProduct detailProduct = new BorrowProductDetailProduct();
                detailProduct.setBorrowProductDetail(detail);

                detail.setProduct((Product)erpDao.queryBySql(objectToSql.generateSelectSqlByAnnotation(new Product(detail.getProductNo())), Product.class).get(0));

                List<BorrowProductDetailProduct> borrowProductDetailProducts = erpDao.query(detailProduct);
                detail.setDetailProducts(new HashSet<>(borrowProductDetailProducts));
            }
            ...
            ele.setActions(erpDao.query(action));
        }

        return borrowProducts;
    }

    public String saveBorrowProductReturn(BorrowProductReturn borrowProductReturn) {
        String result = CommonConstant.fail;

        String msg = isCanSaveBorrowProductReturn(borrowProductReturn);
        if (msg != null) {
            return CommonConstant.fail + msg;
        }

        borrowProductReturn.setState(ErpConstant.borrowProductReturn_state_save);
        borrowProductReturn.setInputDate(dateUtil.getSecondCurrentTimestamp());
        borrowProductReturn.setReturner(getUserBySessionId(borrowProductReturn.getSessionId()));

        result += erpDao.save(borrowProductReturn);
        result += saveBorrowProductReturnDetails(borrowProductReturn);

        return result.equals(CommonConstant.fail) ? result : result.substring(CommonConstant.fail.length());
    }

    public String isCanSaveBorrowProductReturn(BorrowProductReturn borrowProductReturn) {
        String msg = null;

        for (BorrowProductReturnDetail detail : borrowProductReturn.getDetails()) {
            BorrowProductDetail borrowProductDetail = (BorrowProductDetail)erpDao.queryById(detail.getBorrowProductDetail().getId(),
                    detail.getBorrowProductDetail().getClass());

            if (borrowProductDetail.getQuantity().compareTo(detail.getQuantity()) < 0) {
                return  "商品:" + detail.getProductNo() + "还货数量" + detail.getQuantity() + "大于实际借货数量:" + borrowProductDetail.getQuantity() + ",保存失败";
            }
    ...
    public String updateBorrowProductReturn(BorrowProductReturn borrowProductReturn) {
        String result = CommonConstant.fail;
        ...
        if (dbBorrowProductReturn.getState().compareTo(ErpConstant.borrowProductReturn_state_save) == 0) {
            ...
            for (BorrowProductReturnDetail detail : dbBorrowProductReturn.getDetails()) {
                BorrowProductReturnDetailProduct detailProduct = new BorrowProductReturnDetailProduct();
                detailProduct.setBorrowProductReturnDetail(detail);
                List<BorrowProductReturnDetailProduct> detailProducts = erpDao.query(detailProduct);

                for (BorrowProductReturnDetailProduct dbDetailProduct : detailProducts) {
                    result += erpDao.delete(dbDetailProduct);
                }
            ...
            result += CommonConstant.fail + ", 还货单 " + dbBorrowProductReturn.getNo() + " 不是保存状态，不能修改";
        }
    ...
    public String saveBorrowProductReturnDetails(BorrowProductReturn borrowProductReturn) {
        String result = CommonConstant.fail;

        BorrowProductReturn idBorrowProductReturn = new BorrowProductReturn(borrowProductReturn.getId());
        for (BorrowProductReturnDetail detail : borrowProductReturn.getDetails()) {
            detail.setBorrowProductReturn(idBorrowProductReturn);
            result += erpDao.save(detail);
            ...
            List<Product> products = erpDao.query(new Product(detail.getProductNo()));
            BorrowProductReturnDetail idDetail = new BorrowProductReturnDetail(detail.getId());
            for (int i =0; i < detailCount; i++) {
                BorrowProductReturnDetailProduct detailProduct = new BorrowProductReturnDetailProduct();
                detailProduct.setProduct(products.get(i));
                detailProduct.setBorrowProductReturnDetail(idDetail);
                result += erpDao.save(detailProduct);
            }
        }
    ...
    public List<BorrowProductReturn> queryBorrowProductReturn(BorrowProductReturn borrowProductReturn) {
        List<BorrowProductReturn> borrowProductReturns = erpDao.query(borrowProductReturn);

        for (BorrowProductReturn ele : borrowProductReturns) {
            for (BorrowProductReturnDetail detail : ele.getDetails()) {
                BorrowProductReturnDetailProduct detailProduct = new BorrowProductReturnDetailProduct();
                detailProduct.setBorrowProductReturnDetail(detail);

                detail.setProduct((Product)erpDao.queryBySql(objectToSql.generateSelectSqlByAnnotation(new Product(detail.getProductNo())), Product.class).get(0));

                List<BorrowProductReturnDetailProduct> borrowProductReturnDetailProducts = erpDao.query(detailProduct);
                detail.setDetailProducts(new HashSet<>(borrowProductReturnDetailProducts));
            }
        ...
        return borrowProductReturns;
    }

    /**
     * 设置借货单还货状态
     * @param borrowProductReturn
     * @return
     */
    public String setBorrowProductReturned(BorrowProductReturn borrowProductReturn) {
        String result = CommonConstant.fail;

        List<BorrowProductDetail> borrowDetails = new ArrayList<>();
        for (BorrowProductReturnDetail detail : borrowProductReturn.getDetails()) {
            BorrowProductDetail borrowDetail = (BorrowProductDetail) erpDao.queryById(detail.getBorrowProductDetail().getId(), detail.getBorrowProductDetail().getClass());

            if (detail.getQuantity().compareTo(borrowDetail.getQuantity()) == 0) {
                borrowDetail.setState(ErpConstant.borrowProduct_detail_state_returned);
            } else {
                borrowDetail.setState(ErpConstant.borrowProduct_detail_state_returned);
            }

            result += erpDao.updateById(borrowDetail.getId(), borrowDetail);
            boolean isSame = false;
            for (BorrowProductDetail ele : borrowDetails) {
                if (borrowDetail.getId().compareTo(ele.getId()) == 0) {
                    isSame = true;
                    break;
                }
            }
        ...
        for (BorrowProductDetail borrowDetail : borrowDetails) {
            BorrowProduct borrowProduct = (BorrowProduct) erpDao.queryById(borrowDetail.getBorrowProduct().getId(), borrowDetail.getBorrowProduct().getClass());

            int returnedCount = 0, partReturnedCount = 0;
            for (BorrowProductDetail ele : borrowProduct.getDetails()) {
                if (ele.getState().compareTo(ErpConstant.borrowProduct_detail_state_returned) == 0) {
                    returnedCount++;
                }
                if (ele.getState().compareTo(ErpConstant.borrowProduct_detail_state_partReturned) == 0) {
                    partReturnedCount++;
                }
            }

            if (returnedCount == borrowProduct.getDetails().size()) {
                borrowProduct.setState(ErpConstant.borrowProduct_state_returned);
            } else {
                if (partReturnedCount > 0) {
                    borrowProduct.setState(ErpConstant.borrowProduct_state_partReturned);
                }
            }
