...
public interface AfterSaleServiceClient {
    ...
    @RequestMapping(value = "/getProductOnRepairQuantity", method = RequestMethod.POST)
    String getProductOnRepairQuantity(@RequestBody String json);

    @RequestMapping(value = "/getProductRepairedQuantity", method = RequestMethod.POST)
    String getProductRepairedQuantity(@RequestBody String json);

    @RequestMapping(value = "/getLastValidRepairProductByProduct", method = RequestMethod.POST)
    String getLastValidRepairProductByProduct(@RequestBody String json);
    ...
    class OrderClientFallback implements AfterSaleServiceClient {
        ...
        @Override
        public String getProductOnRepairQuantity(String json) {
            logger.info("getProductOnRepairQuantity 异常发生，进入fallback方法，接收的参数：" + json);
            return "{}";
        }

        @Override
        public String getProductRepairedQuantity(String json) {
            logger.info("getProductRepairedQuantity 异常发生，进入fallback方法，接收的参数：" + json);
            return "{}";
        }

        @Override
        public String getLastValidRepairProductByProduct(String json) {
            logger.info("getLastValidRepairProductByProduct 异常发生，进入fallback方法，接收的参数：" + json);
            return "{}";
        }
    }
}
